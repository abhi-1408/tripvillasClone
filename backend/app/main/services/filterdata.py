...
def filter_data(data):
    ...
    for key in data:
        ...
        if key == 'hotel_id':
            ...
            result = result.filter(RoomMeta.airconditioner == data[key])
        ...
        elif key == 'refrigarator':
            result = result.filter(RoomMeta.refrigarator == data[key])
        elif key == 'television':
    ...
    if sort_by:
        if sort_by == 'asc':
            result = result.order_by(RoomMeta.price)
        elif sort_by == 'desc':
            result = result.order_by(RoomMeta.price.desc())
    ...
    for a,b in res:
        ...
        temp_dict['total_units'] = a.total_units
        temp_dict['price'] = b.price
