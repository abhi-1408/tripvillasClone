...
import { Recommend_By_Specific_Hotel } from '../Redux/recommend/action'
...
export const Propertypage = (props) => {
    ...
    const { property_data, message, message_flag, specific_property_flag } = common
    let filter = useSelector((state) => state.filter);
    const { filters } = filter
    let rec = useSelector((state) => state.recommend);
    const { recommend_specific_flag, recommended_specific } = rec
        ...
        if (!auth_logged) {
            ...
            dispatch(LoadBookingData([{
                ...
                "total_cost": {
                    ...
                    "discount": "0",
                    "tax": parseInt(parseFloat(property_data[0]['total_price']) * 0.20).toString(),
                    "cleaning_tax": parseInt(parseFloat(property_data[0]['total_price']) * 0.05).toString(),
                    "total": (parseInt(property_data[0]['total_price']) + parseInt(parseFloat(property_data[0]['total_price']) * 0.20) + parseInt(parseFloat(property_data[0]['total_price']) * 0.05)).toString()
                }
            }]))
                                ...
                                <div style={{ marginTop: '50px' }} className='ml-4 mr-4 ' id="map">
                                    ...
                                    <h3>
                                        <div style={{ width: "60vw", height: "100vh" }}>
                                            <WrappedStaticMap {...{ "lat": item['lat'], "lng": item['lng'] }} googleMapURL={process.env.REACT_APP_GOOGLE_MAPS_KEY}
